...
x-jupyter-template: &jupyter-template
  ...
  # runtime: nvidia 대신 아래 deploy.reservations 사용 권장
  env_file: [.env]
  environment:
    - NVIDIA_DRIVER_CAPABILITIES=compute,utility
  shm_size: 128g
  deploy:
    resources:
      ...
      reservations:
        devices:
          - driver: nvidia
            device_ids: ['0'] # 서비스별로 아래에서 덮어쓰기
  volumes:
    - /raid/shared:/home/jupyter/shared
    - ./jupyter_lab_config.py:/etc/jupyter/jupyter_lab_config.py:ro
...
services:
  jupyter-gpu0:
    <<: *jupyter-template
    container_name: jupyter-gpu0
    ports: ["8888:8888"]
    environment:
      - JUPYTER_PASSWORD=${JUPYTER_PASS_0}
    volumes:
      - /workspace/8888:/home/jupyter
      - /raid/8888:/home/jupyter/extra_data
      - ./jupyter_lab_config.py:/etc/jupyter/jupyter_lab_config.py:ro
      - ./pip.conf:/etc/pip.conf:ro
    deploy:
      resources:
        reservations:
          devices:
            - driver: nvidia
              device_ids: ['0'] # GPU 0번 할당

  jupyter-gpu1:
    <<: *jupyter-template
    container_name: jupyter-gpu1
    ports: ["8889:8888"]
    environment:
      - JUPYTER_PASSWORD=${JUPYTER_PASS_1}
    volumes:
      - /workspace/8889:/home/jupyter
      - /raid/8889:/home/jupyter/extra_data
      - ./jupyter_lab_config.py:/etc/jupyter/jupyter_lab_config.py:ro
      - ./pip.conf:/etc/pip.conf:ro
    deploy:
      resources:
        reservations:
          devices:
            - driver: nvidia
              device_ids: ['1'] # GPU 1번 할당

  jupyter-gpu2:
    <<: *jupyter-template
    container_name: jupyter-gpu2
    ports: ["8890:8888"]
    environment:
      - JUPYTER_PASSWORD=${JUPYTER_PASS_2}
    volumes:
      - /workspace/8890:/home/jupyter
      - /raid/8890:/home/jupyter/extra_data
      - ./jupyter_lab_config.py:/etc/jupyter/jupyter_lab_config.py:ro
      - ./pip.conf:/etc/pip.conf:ro
    deploy:
      resources:
  ...
  jupyter-gpu3:
    <<: *jupyter-template
    container_name: jupyter-gpu3
    ports: ["8891:8888"]
    environment:
      - JUPYTER_PASSWORD=${JUPYTER_PASS_3}
    volumes:
      - /workspace/8891:/home/jupyter
      - /raid/8891:/home/jupyter/extra_data
      - ./jupyter_lab_config.py:/etc/jupyter/jupyter_lab_config.py:ro
      - ./pip.conf:/etc/pip.conf:ro
    deploy:
      resources:
